...
pi = pigpio.pi()
if not pi.connected:
    print "PIGPIO server not located!"
    exit()
...
pi.set_servo_pulsewidth(pTilt, initTilt)
...
def P0():  # Process 0 controls Pan servo
    ...
    while True:
        ...
        if _ServoPanCP < _ServoPanDP:  # if ServoPanCP less than ServoPanDP
            ...
            pi.set_servo_pulsewidth(pPan, _ServoPanCP)
...
def P1():  # Process 1 controls Tilt servo using same logic as above
    speed = .1
    ...
    while True:
        ...
        if _ServoTiltCP < _ServoTiltDP:
            _ServoTiltCP += 1
            ServoTiltCP.put(_ServoTiltCP)
            pi.set_servo_pulsewidth(pTilt, _ServoTiltCP)
...
try:
    while True:
        ...
        # We search for a face three different ways, and if we have found one already-
        ...
        if not faceFound:
            if lastface == 0 or lastface == 1:
                ...
                aframe = capture.grab()  # the webcam consistently gets exactly five frames behind-
        ...
        if not faceFound:  # if we didnt find a face yet...
            if lastface == 0 or lastface == 2:  # only attempt it if we didn't find a face last loop or if-
                ...
                pfacer = profileface.detectMultiScale(aframe, 1.3, 4, (
                        cv2.cv.CV_HAAR_DO_CANNY_PRUNING + cv2.cv.CV_HAAR_FIND_BIGGEST_OBJECT + cv2.cv.CV_HAAR_DO_ROUGH_SEARCH), (80, 80))
        ...
        if not faceFound:  # a final attempt
            if lastface == 0 or lastface == 3:  # this is another profile face search, because OpenCV can only-
                ...
                pfacel = profileface.detectMultiScale(aframe, 1.3, 4, (
                            cv2.cv.CV_HAAR_DO_CANNY_PRUNING + cv2.cv.CV_HAAR_FIND_BIGGEST_OBJECT + cv2.cv.CV_HAAR_DO_ROUGH_SEARCH), (80, 80))
        ...
        if lastface == 3:
            Cface = [(w / 2 + (width - x - w)),
                     (h / 2 + y)]  # we are given an x,y corner point and a width and height, we need the center
        else:
            Cface = [(w / 2 + x),
                     (h / 2 + y)]  # we are given an x,y corner point and a width and height, we need the center
    ...
    pi.set_servo_pulsewidth(pPan, 0)
    pi.set_servo_pulsewidth(pTilt, 0)
    pi.stop()
    capture.release()
