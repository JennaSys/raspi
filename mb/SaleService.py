...
class SaleServiceCalls():
    ...
    def MatchProductOnlinePricesToPrices(self):
        ...
        products = SaleServiceMethods().GetProducts(
            None, None, None, None, None, False).Products.Product
...
class SaleServiceMethods():
    ...
    def CheckoutShoppingCart(self, cartId,
        ...
        request.Test = test
        request.CartItems = BasicRequestHelper.FillArrayType(
            self.service, cartItems, "CartItem", "CartItem")
        request.InStore = inStore
        request.PromotionCode = promoCode
        request.Payments = BasicRequestHelper.FillArrayType(
            self.service, payments, "PaymentInfo", "PaymentInfo")
    ...
    def GetProducts(self, productIds, searchText, searchDomain, categoryIds, subCategoryIds, sellOnline):
        ...
        request.SubCategoryIDs = BasicRequestHelper.FillArrayType(
            self.service, subCategoryIds, "Int")
        request.SellOnline = sellOnline
